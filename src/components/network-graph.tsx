...
const NetworkGraph: React.FC<NetworkGraphProps> = ({ data }) => {
    ...
    useEffect(() => {
        ...
        const sizeScale = d3.scaleLinear()
            ...
            const groupForce = () => {
                const strength = 0.1; // Ajustez l'intensité de la force de regroupement
                return (alpha: number) => {
                    nodes.forEach((node) => {
                        // Trouvez le centre théorique pour chaque groupe
                        ...
                        // Appliquez une force vers le centre du groupe
            ...
            // Ajoutez la force personnalisée dans la simulation
        ...
        const node = svg.append("g")
            ...
            .data(nodes)
            .join("circle")
            .attr("r", d => sizeScale(d.consumption)) // Set radius based on consumption
        ...
        node.append("title")
            ...
            .data(nodes)
            .join("text")
            .attr("dy", d => sizeScale(d.consumption) + 20) // Position text below node
    ...
    >
      ...
      <motion.div
        className="absolute w-12 h-12 bg-transparent outline-border border-[3px] border-neutral-100 rounded-full pointer-events-none drop-shadow-[0_0_10px_rgba(200,255,255,0.8)]"
        style={{
